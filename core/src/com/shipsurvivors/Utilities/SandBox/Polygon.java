...
/*The Vertex class is basically one of the elements of a Doubly linked list, which is called Polygon, and of
course contains the Vertices of a Polygon.*/
class Vertex {
 ...
 * The test assures you that the ray will only travel in one direction in its intersecting path.
 */
class Intersection {
    float x, y;
    float us, uc;
...
public class Polygon {
    ...
    protected void add(Vertex vertex) {
        /*If you're adding the first vertex*/
        if (first == null) {
            first = vertex;
            first.next = vertex;
            first.prev = vertex;
        } else {
            /*if it is not the first it will make it the first
     ...
     */
    /*So, this whole thing what it will return i think is a list of many polygons, and each of those seems to contain a part
    * of the whole clipped polygon, so i'm guessing that in the particular implementation for Box2D they are going to join them
    * or who knows.*/
    protected List<Polygon> clip(Polygon clip, boolean s_entry, boolean c_entry) {
        // Phase one - find intersections and add them to the polygon
        Vertex s = this.first;
        Vertex c = clip.first;
        ...
        /*Yeah it basically does that, it assigns true if an intersection is
        * an entry point to the other Polygon and assigns false if it is an exit from the
        * other Polygon*/
        s = this.first;
        c = clip.first;
